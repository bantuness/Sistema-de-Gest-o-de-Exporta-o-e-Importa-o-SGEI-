...
IF EXISTS (SELECT * FROM SYS.objects WHERE NAME = 'EXPORTACOES' AND type ='U')
	BEGIN
...
IF EXISTS (SELECT * FROM SYS.objects WHERE NAME = 'ITENSIMPORTACAO' AND type ='U')
	BEGIN
		DROP TABLE ITENSIMPORTACAO
	END
GO

CREATE TABLE ITENSIMPORTACAO(
ID_ITEM			INTEGER IDENTITY (1,1) PRIMARY KEY,
ID_IMPORTACAO	INTEGER FOREIGN KEY REFERENCES IMPORTACOES(ID_IMPORTACAO),
ID_PRODUTO		INTEGER FOREIGN KEY REFERENCES PRODUTOS(ID_PRODUTO),
QUANTIDADE		INTEGER)
GO